<script lang="ts">
  ...
  // Type definitions
  type ViewMode = 'flow' | 'output' | 'workspace' | 'calendar' | 'timeline';
  ...
  onMount(async () => {
    ...
    if (siloId && $user) {
      ...
      if ($page.url.pathname.includes('/output')) {
        currentView = 'output';
      } else if ($page.url.pathname.includes('/workspace')) {
  ...
  function setViewMode(mode: ViewMode) {
    ...
    const url = mode === 'flow' ? baseUrl : 
                ...
                mode === 'workspace' ? `${baseUrl}/workspace` :
...
<header class="rounded-lg shadow-sm mx-4 my-3 bg-[var(--bg-secondary)] border-[var(--border-color)]">
  {#if silo}
    <div class="p-3">
      <!-- Project Name and Controls -->
      <div class="flex items-center justify-between mb-3">
        <div class="flex items-center gap-2">
          <a href="/silos" class="text-[var(--text-primary)] hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors duration-300 p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
          ...
          {#if renamingSilo}
            <div class="flex items-center gap-2">
              <input
                ...
                class="bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-600 px-2 py-1 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-500"
                placeholder="Enter name"
                autofocus
              />
              <button 
                on:click={() => { renameSilo(silo.id, newSiloName); renamingSilo = false; }} 
                class="bg-[var(--brand-green)] hover:bg-[var(--brand-dark)] text-[var(--text-primary)] px-2 py-1 rounded-md text-sm transition-colors duration-300">
                Save
              </button>
            </div>
          {:else}
            <div class="flex items-center gap-1 group">
              <h1 class="text-base font-medium text-[var(--text-primary)]">{silo.name}</h1>
              <button 
                on:click={() => renamingSilo = true} 
                class="text-[var(--text-primary)] opacity-0 group-hover:opacity-100 transition-opacity duration-300 hover:text-[var(--bg-primary)] dark:text-[var(--text-primary)] dark:hover:text-gray-200 p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
      ...
      <div class="flex space-x-1 sm:space-x-2 border-b border-[var(--bg-primary)]">
        <button 
          class="tab-button flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-t-md transition-all duration-300 {currentView === 'flow' ? 'text-slate-600 dark:text-slate-500 border-b-2 border-slate-600 dark:border-slate-400 bg-slate-50 dark:bg-slate-900/10' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/30'}" 
        ...
        <button 
          class="tab-button flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-t-md transition-all duration-300 {currentView === 'output' ? 'text-slate-600 dark:text-slate-500 border-b-2 border-slate-600 dark:border-slate-400 bg-slate-50 dark:bg-slate-900/10' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/30'}"
        ...
        <button 
          class="tab-button flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-t-md transition-all duration-300 {currentView === 'workspace' ? 'text-slate-600 dark:text-slate-500 border-b-2 border-slate-600 dark:border-slate-400 bg-slate-50 dark:bg-slate-900/10' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/30'}"
        ...
        <button 
          class="tab-button flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-t-md transition-all duration-300 {currentView === 'timeline' ? 'text-slate-600 dark:text-slate-500 border-b-2 border-slate-600 dark:border-slate-400 bg-slate-50 dark:bg-slate-900/10' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/30'}" 
        ...
        <button 
          class="tab-button flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-t-md transition-all duration-300 {currentView === 'calendar' ? 'text-slate-600 dark:text-slate-500 border-b-2 border-slate-600 dark:border-slate-400 bg-slate-50 dark:bg-slate-900/10' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/30'}"
